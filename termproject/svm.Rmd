...
SalePrice <- all$SalePrice[!is.na(all$SalePrice)]
...
```{r}
svm_model<-svm(SalePrice~., data=train1, cost = 3)
svm_pred <- predict(svm_model,newdata = test1)
svm_pred_unnormalized <- exp(svm_pred)
solution <- data.frame(Id=test_labels,SalePrice=svm_pred_unnormalized)
