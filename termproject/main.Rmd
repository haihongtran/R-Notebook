...
library(Rmisc)
library(ggrepel) # for ggplot2
library(randomForest) # random forest
library(psych) # for psychological
...
##Add more features
```{r}
all$soldHighSeason <- ifelse(all$MoSold %in% c(5,6,7), 1, 0)
all$isRemodeledRecent <- ifelse(all$YearRemodAdd == all$YrSold, 1, 0)
all$averageRoomSizeAbvGrd <- all$GrLivArea / all$TotRmsAbvGrd
all$bathRoomToRoomAbvGrd <- (all$FullBath + all$HalfBath * 0.5) / all$TotRmsAbvGrd
```
...
```{r}
outlierIdx = which(all['GrLivArea']>4000 & all['SalePrice']<300000)
outlierIdx = c(outlierIdx, which(all['OverallQual']<5 & all[,'SalePrice']>200000))
all <- all[-outlierIdx,]
```
...
numericVarNames <- append(numericVarNames, c('Age', 'TotalPorchSF', 'TotBathrooms', 'TotalSqFeet', 'averageRoomSizeAbvGrd', 'bathRoomToRoomAbvGrd'))
...
trControl <- trainControl(
        ...
        index=createResample(train1$OverallQual, 7),
        allowParallel =TRUE
)
...
finalPredictions <- exp(predict(greedyEnsemble, newdata=test1))
```

```{r}
# submit
submission <- data.frame(Id = test_labels, SalePrice=finalPredictions)
head(submission)
